...
<head>
	...
	<?php elseif ( is_404() ) : ?>
		<title><?php _e( 'Page Not Found |', 'titan' ); ?> | <?php bloginfo( 'name'); ?> - restaurant reviews and food thoughts</title>
	<?php elseif ( is_search() ) : ?>
		<title><?php printf(__ ("Search results for '%s'", "titan"), attribute_escape(get_search_query())); ?> | <?php bloginfo( 'name'); ?> - restaurant reviews and food thoughts</title>
	<?php else : ?>
		<title><?php wp_title($sep = ''); ?> | <?php bloginfo( 'name');?> - restaurant reviews and food thoughts</title>
	<?php endif; ?>
    ...
    var _gaq = _gaq || [];
    _gaq.push(['_setAccount', 'UA-18758196-1']);
...
<body <?php body_class(); ?>>
	...
	<div id="header" class="clear">
		...
		<div class="wrapper">
			<?php if (is_home()) echo( '<h1 id="title">'); else echo( '<div id="title">');?><a href="<?php bloginfo( 'url'); ?>"><img src="<?php bloginfo( 'stylesheet_directory'); ?>/images/header_ross.jpg" alt="" width="107" height="84" id="frame" /><?php bloginfo( 'name'); ?></a><?php if (is_home()) echo( '</h1>'); else echo( '</div>');?>
			<div id="description">
